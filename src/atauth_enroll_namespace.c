...
int parse_enroll_namespace(const char *input, struct enroll_namespace *output) {
  ...
  char *buffer = (char *)malloc(sizeof(char) * (input_len + 1));
  if (buffer == NULL) {
  ...
  output->namespaces = (char **)malloc(sep_count * sizeof(char *));
  ...
  output->permissions = (enum namespace_permissions *)malloc(sep_count * sizeof(enum namespace_permissions));
  ...
  for (int i = 0; i < sep_count; i++) {
    ...
    (output->namespaces)[i] = (char *)malloc(sizeof(char) * (namespace_len + 1));
    ...
    case np_readonly:
    case np_read_write:
      output->permissions[i] = (enum namespace_permissions)permission_bits;
      break;
    default:
