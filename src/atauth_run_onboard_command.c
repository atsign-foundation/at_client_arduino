#include "atauth/run_onboard_command.h"
...
int atauth_onboard_command(const char *atsign, const char *root_domain, const char *keys_path, const char *cram_key) {
  ...
  atauth_enroll_params_t ep = {
      .app_name = (char *)ATAUTH_DEFAULT_FIRST_APP_NAME,
      .device_name = (char *)ATAUTH_DEFAULT_FIRST_DEVICE_NAME,
  ...
  if (ret != 0) {
    atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR,
                 "Failed to set is_public=true on atkey for encrypt public key\n");
    goto free_encrypt_public_atkey;
  }
