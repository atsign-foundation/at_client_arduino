...
#ifdef ATCHOPS_TARGET_NATIVE
// stdio-based atlogger_log for native/host builds (not used on Arduino where atsdk_atsdk.cpp overrides it)
void atlogger_log(const char *tag, const enum atlogger_logging_level level, const char *format, ...) {
  ...
  va_start(args, format);
  if (tag != NULL) {
    fprintf(atlogger_logging_stream, "[%s] ", tag);
  }
  vfprintf(atlogger_logging_stream, format, args);
  va_end(args);
}
