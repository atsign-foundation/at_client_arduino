...
// Scrub key material before releasing it: freed heap holding raw keys can
// otherwise surface in later allocations or crash dumps - on a single-address-
// space MCU there is no OS to zero pages between users of the heap.
...
int atauth_apkam_symmetric_key_generate(struct atauth_apkam_symmetric_key *key,
  ...
  key->symmetric_key_raw = (unsigned char *)malloc(sizeof(unsigned char) * (key->symmetric_key_raw_len));
  ...
  key->symmetric_key_base64 = (char *)malloc(sizeof(char) * (key->symmetric_key_base64_len + 1));
  ...
  if (ret != 0) {
    atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR, "Failed to base64 encode symmetric key\n");
    goto exit;
  }
  ...
  key->encrypted_symmetric_key_base64 =
      (char *)malloc(sizeof(char) * (key->encrypted_symmetric_key_base64_len + 1));
  ...
  if (ret != 0) {
    atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR, "Failed to base64 encode encrypted symmetric key\n");
    goto exit;
  }
...
int atauth_generated_first_enrollment_keys_generate(struct atauth_generated_first_enrollment_keys *keys) {
  ...
  keys->self_encryption_key_raw = (unsigned char *)malloc(sizeof(unsigned char) * (keys->self_encryption_key_raw_len));
  ...
  keys->self_encryption_key_base64 = (char *)malloc(sizeof(char) * keys->self_encryption_key_base64_len);
...
int atauth_generated_apkam_enrollment_keys_populate_atkeys(
  ...
  ret = atclient_atkeys_set_apkam_symmetric_key_base64(atkeys,
                                                       generated_keys->apkam_symmetric_key.symmetric_key_base64,
